...
(function () {
  ...
  function init() {
    ...
    var GEOM = {
      H2O: { center: [0, 0, 0], slots: [[Math.sin(0.911), -Math.cos(0.911), 0], [-Math.sin(0.911), -Math.cos(0.911), 0]] }, // 104.5°/2≈52.25°≈0.911rad
      CO2: { center: [0, 0, 0], slots: [[1, 0, 0], [-1, 0, 0]] }, // 직선형
    ...
    function buildLevel() {
      ...
      for (var s = 0; s < g.slots.length; s++) {
        ...
        var dist = bondLen(centerEl, t.atoms[1]);
    ...
    function addAtom(el) {
      ...
      // 결합 차수: CO₂만 C=O 이중결합(order 2), 나머지는 단일(order 1)
      var order = (t.key === 'CO2') ? 2 : 1;
      var newAtom = { el: el, mesh: m, slot: slotIdx };
      // 사전 검사(순수함수): 결합이 원자가를 넘지 않는지
    ...
    function makeBondMesh(p1, p2, order) {
      ...
      // 이중결합은 두 개의 평행 실린더로
      var offsets = order === 2 ? [-0.13, 0.13] : [0];
      // 수직 오프셋 방향
    ...
    function win() {
      ...
      if (lvl >= TARGETS.length) {
        setHud('🏆 모든 분자 완성! 총 ' + score + '점');
        setTimeout(function () { lvl = 0; score = 0; buildLevel(); }, 3000);
      } else {
